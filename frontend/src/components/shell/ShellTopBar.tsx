...
import { ShellAttendanceModal } from '@/components/shell/ShellAttendanceModal';
...
export function ShellTopBar({ showCheckIn, onOpenMobileMenu }: Props) {
  ...
  const { today, refetch } = useTodayAttendance();
      ...
      <header className="sticky top-0 z-30 flex h-12 items-center gap-3 border-b border-gray-200 bg-white px-4">
        ...
        {showCheckIn && (
          <div className="ml-auto shrink-0">
            <ShellCheckInCta model={ctaModel} onOpen={() => setModalOpen(true)} />
          </div>
        )}
      </header>

      {showCheckIn && (
        <ShellAttendanceModal
          visible={modalOpen}
          onHide={() => setModalOpen(false)}
          today={today}
          ctaKind={ctaModel.kind}
          onRefetch={refetch}
        />
      )}
    </>
