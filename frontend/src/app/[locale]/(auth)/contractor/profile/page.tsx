...
import { AlertCircle, Briefcase, RefreshCw, User } from 'lucide-react';
...
export default function ContractorProfilePage() {
  ...
  return (
    <>
      <div>
        <h1 className="text-xl font-semibold text-gray-900">{t('me_title')}</h1>
        <p className="mt-1 text-sm text-gray-500">{t('me_subtitle')}</p>
      </div>

      {offline && <Message severity="warn" text={t('offline_banner')} className="w-full" />}
            ...
            <div className="flex flex-wrap items-start gap-4">
              ...
              <div className="min-w-0 flex-1">
                ...
                <p className="mt-0.5 flex items-center gap-1.5 text-sm text-gray-500">
                  ...
                  {t('contractor_label')}
                  {worker.contractorProfile?.billingModel
                    ? ` · ${t(`billing_model_${worker.contractorProfile.billingModel}`)}`
                    : ''}
                </p>
                {worker.email && <p className="mt-1 text-sm text-gray-500">{worker.email}</p>}
                {worker.contractorProfile?.agencyName && (
                  <p className="mt-1 text-xs text-gray-500">
                    {t('agency_label', { name: worker.contractorProfile.agencyName })}
                  </p>
                )}
              </div>
            </div>
