...
import { AlertCircle, FileSignature, PenLine, RefreshCw } from 'lucide-react';
...
function envelopeSteps(
  ...
  const inProgress = status === 'partially_signed' || status === 'sent' || status === 'completed';
  const completed = status === 'completed';

  return [
    { label: t('tracker_sent'), state: sentDone ? 'done' : 'current' },
    { label: t('tracker_you_sign'), state: completed ? 'done' : inProgress ? 'current' : 'todo' },
    { label: t('tracker_completed'), state: completed ? 'done' : 'todo' },
  ];
}
  ...
  return (
    <>
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="text-xl font-semibold text-gray-900">{t('documents_title')}</h1>
          <p className="mt-1 text-sm text-gray-500">{t('documents_subtitle')}</p>
        </div>
        <Button
          type="button"
          severity="secondary"
      ...
      {!loading && !error && envelopes.length > 0 && (
        <ul className="space-y-3">
          {envelopes.map(envelope => (
            <li key={envelope.id} className="rounded-lg border border-gray-200 bg-white p-4 shadow-sm">
              <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                <div className="min-w-0">
                  <div className="flex flex-wrap items-center gap-2">
                    <h2 className="text-sm font-semibold text-gray-900">{envelope.title}</h2>
                    <Tag value={t(`status_${envelope.status}`)} severity={statusSeverity(envelope.status)} />
                  </div>
